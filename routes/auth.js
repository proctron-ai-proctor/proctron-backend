...
auth.post('/login', async(req, res) => {
  try {
    ...
    else
    {
      ...
      {
        res.status(200).send({auth: true, message: {email: validUser.email, name: validUser.name, token: generateToken({email: validUser.email})}})
      }
      else
      {
...
auth.post('/register', async(req, res) => {
  try {
    ...
    if(!doc.exists)
    {
      ...
      res.status(200).send({auth: false, message: "email sent", payload: JSON.stringify(payload)})
    }
    else
    {
  ...
  try{
    ...
    if(new Date().toISOString() < decoded.expirationTime) {
      if(email == decoded.user.email) {
        if(otp == decoded.otp) {
          await ref.collection('users').doc(req.body.email).set(decoded.user)
